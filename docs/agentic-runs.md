...
## Concurrent arm commands: recorded CLI example

In the September 16, 2026 shoelace run (`20260916-100517-3d3c60c1`), the agent
submitted the following two `joint_target` requests through the recorder. Both
returned `completed`. Recorder request times were 61.149589 s (left) and
61.149917 s (right); response times were 66.213004 s and 66.224068 s respectively.
These overlapping request/response intervals show concurrent execution, not
synchronized motor start times or exact target attainment.

The JSON bodies below are the actual requests. These are historical poses, not
startup or recovery instructions: choose targets from fresh feedback and scene
clearance for the current task. Use the recorder URL supplied by initialization
(the recorded run used port 8771), with a recording and healthy arm sessions
already active. Run from the repository root.

```bash
mkdir -p outputs/concurrent-example
cat > outputs/concurrent-example/left.json <<'JSON'
{"action":{"arm":"left","kind":"joint_target","joints_rad":[-0.25,0.35,0.5,0,0,0],"duration_s":5}}
JSON
cat > outputs/concurrent-example/right.json <<'JSON'
{"action":{"arm":"right","kind":"joint_target","joints_rad":[0.25,0.35,0.5,0,0,0],"duration_s":5}}
JSON

recorder_url=http://127.0.0.1:8771/mcp
uv run robot-call execute --url "$recorder_url" \
  --arguments outputs/concurrent-example/left.json \
  --output outputs/concurrent-example/left-result.json &
left_pid=$!
uv run robot-call execute --url "$recorder_url" \
  --arguments outputs/concurrent-example/right.json \
  --output outputs/concurrent-example/right-result.json &
right_pid=$!

# Wait for both even if one CLI process exits nonzero (including under set -e).
left_exit=0
right_exit=0
wait "$left_pid" || left_exit=$?
wait "$right_pid" || right_exit=$?
printf 'CLI exit codes: left=%s right=%s\n' "$left_exit" "$right_exit"
cat outputs/concurrent-example/left-result.json
cat outputs/concurrent-example/right-result.json
```

This preserves the logged launch pattern: both background processes start before
either `wait`. Paths are shortened, the run-specific `UV_CACHE_DIR` override is
omitted, and exit codes are captured so a failure cannot skip the second wait.
Read both results, including `status`, `diagnostics`, measured feedback, and
`post_action` images. A missing result or transport failure requires a status
check before another action; do not blindly resend it. If needed, obtain a fresh
view after both calls finish:

```bash
uv run robot-call observe --url "$recorder_url" \
  --output outputs/concurrent-example/after-both.json
```

One arm's rejection or stop does not stop the other. The collision model does not
check between arms, and a post-action image can show the other arm still moving.
The same dispatch pattern accepts other supported action kinds; this recorded
example specifically used joint targets.

Local evidence (ignored run artifacts, not required to use these commands):
`outputs/shoelace-cli-20260916/approach-{left,right}.json` and corresponding
`approach-{left,right}-result.json` files; recorder `events.jsonl` in the rollout
above, request IDs `8499c3993c9d42a4ae64b39c82e2bb17` (left) and
`588f5bf8a68c480895cf3342ea6b2372` (right). The CLI transcript records the launch
at 2026-09-16 17:06:18 UTC.
